...
Projectile::Projectile(Vector3D pos,  Vector3D acel, float damping, float masaReal, Vector3D velReal, Vector3D velSimulada, Vector3D grReal) : Particle(pos, velSimulada, acel, damping, 10.0f), masaR(masaReal), velR(velReal), velS(velSimulada),grR(grReal)
{
